...
{
	int len = strnlen(filename, INJECTBYTE_MAX_PATH);
...
static int pathparser_get_drive_by_path(const char** path)
{
	...
	// Add 3 bytes to skip the drive number 0:/ 1:/ 2:/ etc
	*path += 3;
	return drive_no;
...
{
	char* result_path_part = kzalloc(INJECTBYTE_MAX_PATH);
	int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
	...
	if (strlen(path) > INJECTBYTE_MAX_PATH)
	{
		goto out;
	}
