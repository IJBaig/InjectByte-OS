...
struct filesystem* filesystems[INJECTBYTE_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[INJECTBYTE_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_filesystem()
{
	int i = 0;
	for(i = 0; i < INJECTBYTE_MAX_FILESYSTEMS; i++)
	{
		if(filesystems[i] == 0)
		{
...
{
	int res = -ENOMEM;
	for(int i = 0; i < INJECTBYTE_MAX_FILE_DESCRIPTORS; i++)
	{
		if(file_descriptors[i] == 0)
		{
			struct file_descriptor* desc = kzalloc(sizeof(struct file_descriptor));
			//descripors start at 1
			desc->index = i+1;
...
{
	if(fd <= 0 || fd > INJECTBYTE_MAX_FILE_DESCRIPTORS)
	{
		return 0;
	}
...
struct filesystem* fs_resolve(struct disk* disk)
{
	...
	for(int i = 0; i < INJECTBYTE_MAX_FILESYSTEMS; i++)
	{
...
int fopen(const char* filename, const char* mode_string)
{
    ...
    mode = file_get_mode_by_string(mode_string);
